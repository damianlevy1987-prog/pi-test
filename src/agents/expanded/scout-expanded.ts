...
export class ExpandedScout {
  ...
  getSubAgents(): SubAgent[] { return this.subAgents; }
  getSubAgent(name: string): SubAgent | undefined { return this.subAgents.find(s => s.name === name); }
  async execute(subAgentName: string, params: any): Promise<any> {
    const subAgent = this.getSubAgent(subAgentName);
    if (!subAgent) throw new Error(`Sub-agent ${subAgentName} not found`);
    return subAgent.execute(params);
  }
}
...
export class ExpandedVulnHunter {
  private subAgents: SubAgent[] = [
    ...
    { name: 'supply_chain_scan', description: 'Supply chain vulnerability scan', execute: async () => {} },
